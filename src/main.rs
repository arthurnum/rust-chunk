...
fn draw_rectangle_sample(gl: &Gl) -> GLuint {
    unsafe {
        ...
        // let vptr = &vertices as *const _ as *const std::os::raw::c_void;

        // gl.BufferData(ARRAY_BUFFER, 4*12, vptr, STATIC_DRAW);
...
fn main() {
    ...
    let window = video_subsys.window("Title", 600, 400).opengl().build().unwrap();
